...
function ArrowSquare({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`inline-flex items-center justify-center w-8 h-8 bg-[#F6E7BC] text-black rounded-sm shadow ${className}`}
    >
      {/* Chevron in a square */}
      <svg
    ...
    return (
        ...
        <div className="relative rounded-xl overflow-hidden shadow-[0_8px_24px_rgba(0,0,0,0.25)] bg-[#0992C2]">
            {/* CLOSED STATE (image with bottom bar + arrow) */}
            <div
            className={`relative transition-transform duration-300 ${
                open ? "pointer-events-none opacity-70  inset-0" : "opacity-100"
            }`}
            >
            ...
            // height={450}
                ...
                {/* Bottom bar */}
                <div className="absolute bottom-0 left-0 w-full h-8 bg-[#0992C2]"></div>
    ...
    <section id="aktivitas" className="font-['Be_Vietnam_Pro'] bg-[#0B2D72] text-white">
      <div className="mx-auto max-w-7xl px-5 md:px-15 py-12 lg:pt-20 md:py-16">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:gap-12">
          <h2 className="text-[#F6E7BC] text-3xl md:text-4xl font-extrabold md:min-w-[240px] md:leading-tight">
            Aktivitas{" "}
